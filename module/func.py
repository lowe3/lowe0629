...
def manageForm(event, mtext, user_id):
	try:
		...
		for fitems in food.objects.filter(items=eitems):
			...
			timess = int('1')
			unit = eat.objects.create(uid=user_id, bmr=user.objects.get(uid=user_id).bmr, tdee=user.objects.get(uid=user_id).tdee, datetime=edate, items=eitems, calories=fitems.calories, total=fitems.calories, times=timess)  #寫入資料庫
			unit.save()
