			...
			if isinstance(event, MessageEvent):
				if isinstance(event.message, TextMessage):
					...
					if mtext == '好':
						...
						line_bot_api.reply_message(event.reply_token, TextSendMessage(text='好的先生，接下來請選擇您的運動頻率好嗎?'))
					elif mtext == '女性':
						line_bot_api.reply_message(event.reply_token, TextSendMessage(text='好的小姐，接下來請選擇您的運動頻率好嗎?'))
					elif mtext == '推薦菜單':
