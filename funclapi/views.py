			...
			if isinstance(event, MessageEvent):
				if isinstance(event.message, TextMessage):
					...
					if mtext == '好':
						...
						mb = 66+(13*we)+(5*he)-(6*ye)
						line_bot_api.reply_message(event.reply_token, TextSendMessage(text='好的先生，您的基礎代謝率為'+mb+'，接下來請輸入您的運動頻率好嗎?'))
					elif mtext == '女性':
						wb = 655+(9.6*we)+(1.8*he)-(4.7*ye)
						line_bot_api.reply_message(event.reply_token, TextSendMessage(text='好的小姐，您的基礎代謝率為'+wb+'，接下來請輸入您的運動頻率好嗎?'))
					elif mtext == '推薦菜單':
