			...
			if isinstance(event, MessageEvent):
				if isinstance(event.message, TextMessage):
					...
					if mtext == '好':
						...
						line_bot_api.reply_message(event.reply_token, TextSendMessage(text='好的先生，接下來請輸入您的身高，EX：180公分'))
						if mtext[2] == '歲':
							ye1 = ''.join([x for x in mtext if x.isdigit()])
							line_bot_api.reply_message(event.reply_token, TextSendMessage(text='您的年齡是'+ye+'歲，感謝您的告知，接下請選擇您的運動頻率好嗎?'))
						elif mtext[-2:] == '公分':
							he1 = ''.join([x for x in mtext if x.isdigit()])
							line_bot_api.reply_message(event.reply_token, TextSendMessage(text='您的身高是'+he+'公分，請輸入您的體重，EX:70公斤'))
						elif mtext[-2:] == '公斤':
							we1 = ''.join([x for x in mtext if x.isdigit()])
							line_bot_api.reply_message(event.reply_token, TextSendMessage(text='您的體重是'+we+'公斤，請輸入您的年齡，EX：18歲'))
					elif mtext == '女性':
						line_bot_api.reply_message(event.reply_token, TextSendMessage(text='好的小姐，接下來請輸入您的身高，EX：150公分'))					
						if mtext[2] == '歲':
							ye2 = ''.join([x for x in mtext if x.isdigit()])
							line_bot_api.reply_message(event.reply_token, TextSendMessage(text='您的年齡是'+ye+'歲，感謝您的告知，接下請選擇您的運動頻率好嗎?'))
						elif mtext[-2:] == '公分':
							he2 = ''.join([x for x in mtext if x.isdigit()])
							line_bot_api.reply_message(event.reply_token, TextSendMessage(text='您的身高是'+he+'公分，請輸入您的體重，EX:50公斤'))
						elif mtext[-2:] == '公斤':
							we2 = ''.join([x for x in mtext if x.isdigit()])
							line_bot_api.reply_message(event.reply_token, TextSendMessage(text='您的體重是'+we+'公斤，請輸入您的年齡，EX：18歲'))
					elif mtext == '推薦菜單':
